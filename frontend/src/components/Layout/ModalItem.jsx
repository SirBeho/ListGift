...
export default function ModalItem({ selectedItem, show = false, onClose = () => { }, color1 = "#ffffff", color2 = "#ffffff" }) {
   ...
   /*  useEffect(() => {
    ...
    const handleGiftClick = () => {
        setShowConfirmation(true); // Abrir el segundo modal
    };

    const handleConfirmGift = (message) => {
        console.log(`Regalando ${selectedItem.name} con mensaje: ${message}`);
        // 🚨 Aquí iría la lógica para enviar la solicitud PUT/PATCH al Backend con el status: 2
        instance.patch(`/items/${selectedItem.id}`, { status: 2, gift_message: message })
        
        // Cerrar ambos modales después de confirmar
    ...
    return (
        <AnimatePresence>
            {show && ( // Usamos AnimatePresence y movemos 'open' a la condición de AnimatePresence
            <Dialog
                as="div"
                id="modal"
                open={show}
